...
def txt_int(text):
    text = given_num.split("\n")
    for x in range(0,len(text)):
        text[x] = text[x].split()
        for k in range(0,len(text[x])):
            text[x][k]= int(text[x][k])
    return text


def adjacient_of_4(nAr):
    prod = res = 0
    for row in range(0, 20):
        for col in range(0, 20):
            if row <= 16:
                res = nAr[row][col]*nAr[row+1][col]*nAr[row+2][col]*nAr[row+3][col]
                if prod < res: prod = res
            if col<= 16:
                res = nAr[row][col]*nAr[row][col+1]*nAr[row][col+2]*nAr[row][col+3]
                if prod < res: prod = res
